...
void main() {
  test('api client invalidates broken sessions after refresh fails', () {
    ...
    expect(apiClient, contains('!_isRefreshing && _shouldInvalidateSession(e)'));
  ...
  test('auth provider reacts to session invalidation event', () {
    ...
    expect(
      provider,
      contains('SessionInvalidator.listenable.addListener(_handleSessionInvalidated)'),
    );
    expect(
      provider,
      contains('SessionInvalidator.listenable.removeListener(_handleSessionInvalidated)'),
    );
