...
void main() {
  ...
  test('conviteRemainingLabel', () {
    ...
    expect(
      conviteRemainingLabel(now.subtract(const Duration(minutes: 1)), now),
      'Expirado',
    );
    expect(
  ...
  test('conviteAindaValido', () {
    ...
    expect(
      conviteAindaValido(now.add(const Duration(minutes: 1)), now),
      isTrue,
    );
    expect(
  ...
  test('conviteVigenteHint explica link oculto e e-mail preso', () {
    expect(conviteVigenteHint(linkVisivel: true), startsWith('Um uso.'));
    expect(
      conviteVigenteHint(linkVisivel: false, email: 'a@b.com'),
      allOf(startsWith('Só para a@b.com'), contains('Gere outro')),
    );
  });

  test('conviteCadastroEmailDica', () {
    expect(conviteCadastroEmailDica(null), isNull);
    expect(conviteCadastroEmailDica('  '), isNull);
    expect(conviteCadastroEmailDica('a***@b.com'), contains('a***@b.com'));
  });

  test('conviteEmailInvalido aceita vazio e recusa formato ruim', () {
    expect(conviteEmailInvalido(''), isNull);
    expect(conviteEmailInvalido('aluno@exemplo.com'), isNull);
    expect(conviteEmailInvalido('aluno@'), 'E-mail inválido');
  });
