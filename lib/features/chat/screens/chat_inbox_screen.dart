...
final chatInboxUnreadProvider = FutureProvider<List<ChatInboxItem>>((ref) async {
  return ChatRepository(ref.read(apiClientProvider)).inboxUnread();
});

final chatInboxArchivedProvider = FutureProvider<List<ChatInboxItem>>((ref) async {
...
class _ChatInboxScreenState extends ConsumerState<ChatInboxScreen>
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      final results = await ChatRepository(ref.read(apiClientProvider))
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Digite para buscar...', style: TextStyle(color: mute)),
                    ],
                  ),
                )
              : TabBarView(
                  controller: _tabCtrl,
                  children: [
                    _buildInboxTab(ref.watch(chatInboxProvider), chatInboxProvider, isDark, primary, ink, mute),
                    _buildInboxTab(ref.watch(chatInboxUnreadProvider), chatInboxUnreadProvider, isDark, primary, ink, mute, emptyMsg: 'Nenhuma mensagem não lida'),
                    _buildInboxTab(ref.watch(chatInboxArchivedProvider), chatInboxArchivedProvider, isDark, primary, ink, mute, emptyMsg: 'Nenhuma conversa arquivada', isArchived: true),
                  ],
                ),
    );
  }

  Widget _buildSearchResults(bool isDark, Color primary, Color ink, Color mute) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _SearchResultTile(msg: msg, isDark: isDark, ink: ink, mute: mute);
      },
    );
  }
  ...
  Widget _buildInboxTab(
    ...
    return async.when(
      ...
      data: (items) {
        if (items.isEmpty) {
          return _InboxState(
            icon: isArchived ? Icons.archive_outlined : Icons.chat_bubble_outline,
            title: emptyMsg,
            message: isArchived
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _ActionTile(icon: Icons.push_pin, label: 'Fixar conversa', onTap: () {
              Navigator.pop(ctx);
              _conversationAction(item.alunoId, 'pin');
            }),
            _ActionTile(icon: Icons.archive_outlined, label: 'Arquivar', onTap: () {
              Navigator.pop(ctx);
              _conversationAction(item.alunoId, 'archive');
            }),
            _ActionTile(icon: Icons.notifications_off_outlined, label: 'Silenciar', onTap: () {
              Navigator.pop(ctx);
...
class _SearchResultTile extends StatelessWidget {
  ...
  final Color ink;
  final Color mute;
  const _SearchResultTile({required this.msg, required this.isDark, required this.ink, required this.mute});
            ...
            Row(
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(fontSize: 11, color: mute, fontWeight: FontWeight.w600),
                ),
                const Spacer(),
                Text(
...
class _InboxTile extends StatelessWidget {
  ...
  const _InboxTile({required this.item, required this.isDark, required this.onTap, this.onLongPress});
                    ...
                    children: [
                      Expanded(
                        child: Text(
                          ...
                          style: TextStyle(
                            color: ink,
                            fontWeight: item.naoLidas > 0 ? FontWeight.w700 : FontWeight.w600,
                            fontSize: 15,
                          ),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          fontSize: 11,
                          fontWeight: item.naoLidas > 0 ? FontWeight.w600 : FontWeight.w400,
                        ),
                      ),
                    ],
                  ),
                      ...
                      Expanded(
                        child: Text(
                          ...
                          style: TextStyle(
                            ...
                            fontSize: 13,
                            fontWeight: item.naoLidas > 0 ? FontWeight.w500 : FontWeight.w400,
                          ),
                        ),
                      ),
                      if (item.naoLidas > 0) ...[
                        ...
                        Container(
                          ...
                          child: Text(
                            item.naoLidas > 99 ? '99+' : item.naoLidas.toString(),
