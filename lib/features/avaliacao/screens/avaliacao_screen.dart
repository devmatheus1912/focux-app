...
class _AvaliacaoScreenState extends ConsumerState<AvaliacaoScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final r = await AvaliacaoRepository(
  ...
  Future<void> _excluir(AvaliacaoFisica a) async {
    final confirmar = await showDialog<bool>(
      ...
      builder:
          (_) => AlertDialog(
            ...
            content: Text(
              'Deseja excluir a avaliação de ${_formatarData(a.avaliadoEm)}?',
            ),
            actions: [
              TextButton(
    ...
    try {
      await AvaliacaoRepository(
        ref.read(apiClientProvider),
      ).excluir(widget.alunoId, a.id);
      if (mounted) {
  ...
  Future<void> _editar(AvaliacaoFisica a) async {
    await showModalBottomSheet(
      ...
      builder:
          (_) => _EditarAvaliacaoSheet(alunoId: widget.alunoId, avaliacao: a),
    );
    _load();
  }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            children: [
                              if (a.pesoKg != null)
                                _stat('Peso', '${a.pesoKg} kg'),
                              if (a.alturaCm != null)
                                _stat('Altura', '${a.alturaCm} cm'),
                              if (a.imc != null)
                                _stat('IMC', a.imc!.toStringAsFixed(1)),
                              if (a.percGordura != null)
                                _stat('% Gordura', '${a.percGordura}%'),
                              if (a.percentualGordura != null)
                                _stat(
                                  '% Gordura V2',
                                  '${a.percentualGordura}%',
                                ),
                              if (a.percMassa != null)
                                _stat('% Massa', '${a.percMassa}%'),
                              if (a.massaMuscular != null)
                                _stat('Massa Musc.', '${a.massaMuscular} kg'),
                              if (a.cinturaCm != null)
                                _stat('Cintura', '${a.cinturaCm} cm'),
                              if (a.circCintura != null)
                                _stat('Circ. Cintura', '${a.circCintura} cm'),
                              if (a.quadrilCm != null)
                                _stat('Quadril', '${a.quadrilCm} cm'),
                              if (a.circQuadril != null)
                                _stat('Circ. Quadril', '${a.circQuadril} cm'),
                              if (a.circBraco != null)
                                _stat('Circ. Braço', '${a.circBraco} cm'),
                              if (a.circCoxa != null)
                                _stat('Circ. Coxa', '${a.circCoxa} cm'),
                            ],
                          ),
                          if (a.observacoesAvaliacao != null &&
                              a.observacoesAvaliacao!.isNotEmpty) ...[
                            const SizedBox(height: 8),
                            Text(
                              a.observacoesAvaliacao!,
                              style: const TextStyle(
                                color: EagleTokens.inkMute,
                                fontSize: 12,
                              ),
...
class _EditarAvaliacaoSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_EditarAvaliacaoSheet> createState() =>
      _EditarAvaliacaoSheetState();
}
...
class _EditarAvaliacaoSheetState extends ConsumerState<_EditarAvaliacaoSheet> {
  ...
  void initState() {
    ...
    _percGordura = TextEditingController(
      text: a.percentualGordura?.toString() ?? '',
    );
    _massaMuscular = TextEditingController(
      text: a.massaMuscular?.toString() ?? '',
    );
  ...
  void dispose() {
    ...
    _obs.dispose();
    _imc.dispose();
    _percGordura.dispose();
    _massaMuscular.dispose();
    _circCintura.dispose();
    _circQuadril.dispose();
    _circBraco.dispose();
    _circCoxa.dispose();
    _obsAvaliacao.dispose();
    super.dispose();
  }
  ...
  Future<void> _salvar() async {
    ...
    try {
      await AvaliacaoRepository(
        ref.read(apiClientProvider),
      ).editar(widget.alunoId, widget.avaliacao.id, {
        ...
        if (_gordura.text.isNotEmpty)
          'percGordura': double.tryParse(_gordura.text),
        if (_massa.text.isNotEmpty) 'percMassa': double.tryParse(_massa.text),
        if (_cintura.text.isNotEmpty)
          'cinturaCm': double.tryParse(_cintura.text),
        if (_quadril.text.isNotEmpty)
          'quadrilCm': double.tryParse(_quadril.text),
        if (_obs.text.isNotEmpty) 'observacoes': _obs.text,
        if (_imc.text.isNotEmpty) 'imc': double.tryParse(_imc.text),
        if (_percGordura.text.isNotEmpty)
          'percentualGordura': double.tryParse(_percGordura.text),
        if (_massaMuscular.text.isNotEmpty)
          'massaMuscular': double.tryParse(_massaMuscular.text),
        if (_circCintura.text.isNotEmpty)
          'circCintura': double.tryParse(_circCintura.text),
        if (_circQuadril.text.isNotEmpty)
          'circQuadril': double.tryParse(_circQuadril.text),
        if (_circBraco.text.isNotEmpty)
          'circBraco': double.tryParse(_circBraco.text),
        if (_circCoxa.text.isNotEmpty)
          'circCoxa': double.tryParse(_circCoxa.text),
        if (_obsAvaliacao.text.isNotEmpty)
          'observacoesAvaliacao': _obsAvaliacao.text,
      });
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Avaliação atualizada!')));
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _num(_peso, 'Peso (kg)'),
            _num(_altura, 'Altura (cm)'),
            _num(_imc, 'IMC'),
            _secao('Composição Corporal'),
            _num(_gordura, '% Gordura (legacy)'),
            _num(_percGordura, '% Gordura'),
            _num(_massa, '% Massa muscular'),
            _num(_massaMuscular, 'Massa muscular (kg)'),
            _secao('Circunferências (cm)'),
            _num(_cintura, 'Cintura'),
            _num(_circCintura, 'Circ. Cintura'),
            _num(_quadril, 'Quadril'),
            _num(_circQuadril, 'Circ. Quadril'),
            _num(_circBraco, 'Circ. Braço'),
            _num(_circCoxa, 'Circ. Coxa'),
            _secao('Observações'),
...
class _NovaAvaliacaoScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_NovaAvaliacaoScreen> createState() =>
...
class _NovaAvaliacaoScreenState extends ConsumerState<_NovaAvaliacaoScreen> {
  final _peso = TextEditingController(),
      ...
      _gordura = TextEditingController(),
      _massa = TextEditingController(),
      _cintura = TextEditingController(),
      _quadril = TextEditingController(),
      _obs = TextEditingController();
  final _imc = TextEditingController(),
      _percGordura = TextEditingController(),
      _massaMuscular = TextEditingController(),
      _circCintura = TextEditingController(),
      _circQuadril = TextEditingController(),
      _circBraco = TextEditingController(),
      _circCoxa = TextEditingController(),
      _obsAvaliacao = TextEditingController();
  bool _saving = false;
