...
class DepoimentoModel {
  ...
  DepoimentoModel({required this.id, required this.nomeAluno, this.fotoAluno,
    required this.texto, required this.nota, required this.aprovado});
...
class DepoimentoRepository {
  ...
  Future<DepoimentoModel> submeter({required String texto, required int nota}) async {
    final r = await _dio.post('/api/depoimentos', data: {'texto': texto, 'nota': nota});
  ...
  Future<List<DepoimentoModel>> listarMeus() async {
    ...
    return (r.data as List).map((e) => DepoimentoModel.fromJson(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<void> aprovar(int id, {required bool aprovado}) async {
    await _dio.put('/api/personal/depoimentos/$id/aprovar', queryParameters: {'aprovado': aprovado});
  }
}
