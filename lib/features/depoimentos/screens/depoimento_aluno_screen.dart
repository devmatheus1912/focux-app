...
class _State extends ConsumerState<DepoimentoAlunoScreen> {
  ...
  Future<void> _enviar() async {
    ...
    try {
      await DepoimentoRepository(ref.read(apiClientProvider))
      ...
      body: _enviado
          ? Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(
              mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.check_circle_outline, color: EagleTokens.good, size: 72),
                const SizedBox(height: 16),
                Text('Depoimento enviado!', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
                const SizedBox(height: 8),
                Text('Aguardando aprovação do seu personal.', textAlign: TextAlign.center,
                  style: TextStyle(color: isDark ? EagleTokens.darkInkMute : EagleTokens.inkMute)),
                const SizedBox(height: 32),
                OutlinedButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Voltar')),
              ])))
          : SingleChildScrollView(padding: const EdgeInsets.all(24), child: Form(key: _formKey, child: Column(
              crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('AVALIAÇÃO', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                  letterSpacing: 1.2, color: isDark ? EagleTokens.darkInkMute : EagleTokens.inkMute)),
                const SizedBox(height: 16),
                ...
                Text('DEPOIMENTO', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                  letterSpacing: 1.2, color: isDark ? EagleTokens.darkInkMute : EagleTokens.inkMute)),
                const SizedBox(height: 8),
                TextFormField(controller: _textoCtrl, maxLines: 5, maxLength: 500,
                  decoration: const InputDecoration(
                    hintText: 'Conte como foi sua experiência com seu personal trainer...',
