...
class _State extends ConsumerState<DepoimentosPersonalScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final items = await DepoimentoRepository(ref.read(apiClientProvider)).listarParaPersonal();
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            Row(children: List.generate(5, (si) => Icon(
                              si < d.nota ? Icons.star : Icons.star_border,
                              size: 13, color: const Color(0xFFF59E0B)))),
                          ])),
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                            decoration: BoxDecoration(
                              color: d.aprovado ? EagleTokens.good.withValues(alpha: 0.15) : const Color(0xFFF59E0B).withValues(alpha: 0.15),
                              borderRadius: BorderRadius.circular(20)),
                            child: Text(d.aprovado ? 'Aprovado' : 'Pendente',
                              style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                                color: d.aprovado ? EagleTokens.good : const Color(0xFFF59E0B)))),
                        ]),
                        ...
                        const SizedBox(height: 10),
                        Row(children: [
                          if (!d.aprovado) Expanded(child: FilledButton.icon(
                            icon: const Icon(Icons.check, size: 16), label: const Text('Aprovar'),
                          ...
                          Expanded(child: OutlinedButton.icon(
                            icon: Icon(d.aprovado ? Icons.close : Icons.close, size: 16),
                            label: Text(d.aprovado ? 'Remover' : 'Rejeitar'),
