...
class _State extends ConsumerState<GaleriaScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { _fotos = items; _loading = false; });
  ...
  Future<void> _add() async {
    ...
    try {
      final request = http.MultipartRequest('POST', Uri.parse('https://api.cloudinary.com/v1_1/focux/image/upload'))
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.transparent, elevation: 0,
        ...
        actions: [
          if (_uploading) const Padding(padding: EdgeInsets.all(16),
      ...
      body: _loading ? const FxLoading()
          : _fotos.isEmpty
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 16),
                  FilledButton.icon(icon: const Icon(Icons.add_photo_alternate_outlined),
                    label: const Text('Adicionar foto'), onPressed: _add),
                ]))
              : GridView.builder(
