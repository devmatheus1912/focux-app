...
class _AnamneseScreenState extends ConsumerState<AnamneseScreen>
  ...
  void dispose() {
    ...
    _objetivoCtrl.dispose(); _lesoesCtrl.dispose();
    _medicCtrl.dispose(); _obsCtrl.dispose();
    _historicoCtrl.dispose(); _cirurgiasCtrl.dispose(); _doresCtrl.dispose();
    _objDetalhadoCtrl.dispose(); _prefTreinoCtrl.dispose(); _restricoesCtrl.dispose();
    super.dispose();
  }

  Future<void> _load() async {
    try {
      final a = await AnamneseRepository(ref.read(apiClientProvider)).buscar(widget.alunoId);
      _objetivoCtrl.text = a.objetivo ?? '';
      _nivelAtividade = _niveis.contains(a.nivelAtividade) ? a.nivelAtividade : null;
  ...
  Future<void> _salvar() async {
    ...
    try {
      await AnamneseRepository(ref.read(apiClientProvider)).salvar(widget.alunoId, {
  ...
  Widget _tabTreinoNutricao() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        Expanded(child: Text('Disponibilidade semanal: $_dispSemanal dias/semana',
